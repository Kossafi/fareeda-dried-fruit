...
export const requireBranchAccess = (req: Request, res: Response, next: NextFunction): void => {
  ...
  const requestedBranchId = req.params.branchId || req.body.fromBranchId || req.body.toBranchId;
...
export const requireDriverRole = (req: Request, res: Response, next: NextFunction): void => {
  if (!req.user) {
    res.status(401).json({
      success: false,
      message: 'Authentication required',
  ...
  if (req.user.role !== 'driver' && req.user.role !== 'super_admin' && req.user.role !== 'manager') {
    res.status(403).json({
      success: false,
