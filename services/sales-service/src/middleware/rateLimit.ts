...
// Specialized rate limiter for high-frequency operations (like real-time dashboard)
export const highFrequencyRateLimit = async (
  ...
  try {
    ...
    const maxHfRequests = Math.floor(config.rateLimit.max / 4); // 25% of normal limit
